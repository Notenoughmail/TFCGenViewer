...
public enum VisualizerType {
    BIOMES(0b00100000, name("biomes"), (x, y, xPos, zPos, generator, region, point, image) -> setPixel(image, x, y, BiomeColors.get(point.biome)), BiomeColors.colorKey()),
    RAINFALL(0b10000000, name("rainfall"), (x, y, xPos, zPos, generator, region, point, image) -> {
        if (point.land()) {
    ...
    TEMPERATURE(0b10000000, name("temperature"), (x, y, xPos, zPos, generator, region, point, image) -> {
        if (point.land()) {
    ...
    BIOME_ALTITUDE(0b00010000, name("biome_altitude"), (x, y, xPos, zPos, generator, region, point, image) -> {
        if (point.land()) {
    ...
    INLAND_HEIGHT(0b00100000, name("inland_height"), (x, y, xPos, zPos, generator, region, point, image) -> setPixel(image, x, y, InlandHeightColors.color(point)), InlandHeightColors.colorKey()),
    RIVERS(0b00010000, name("rivers_and_mountains"), (x, y, xPos, zPos, generator, region, point, image) -> {
        if (point.land()) {
            final int color;
            if (point.mountain()) {
    ...
    ROCK_TYPES(0b01000000, name("rock_types"), (x, y, xPos, zPos, generator, region, point, image) -> {
    ...
    ROCKS(0b01000000, name("rocks"), (x, y, xPos, zPos, generator, region, point, image) -> {
