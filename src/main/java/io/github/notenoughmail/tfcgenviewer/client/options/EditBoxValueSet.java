...
public record EditBoxValueSet(Supplier<Font> font, Consumer<EditBox> onCreate) implements OptionInstance.ValueSet<String> {
    ...
    public Function<OptionInstance<String>, AbstractWidget> createButton(OptionInstance.TooltipSupplier<String> tooltipSupplier, Options options, int x, int y, int width, Consumer<String> onValueChanged) {
        return i -> {
            final EditBox editBox = new EditBox(font.get(), x, y, width, 20, i.caption) {
                @Override
